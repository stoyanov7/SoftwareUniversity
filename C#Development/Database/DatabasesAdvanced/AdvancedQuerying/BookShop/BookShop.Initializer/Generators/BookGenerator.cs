namespace BookShop.Initializer.Generators
{
    ...
    public static class BookGenerator
    {
        ...
        public static IEnumerable<Book> CreateBooks()
        {
            ...
            var categories = CategoryGenerator.CreateCategories();
            var authors = AuthorGenerator.CreateAuthors();
            var books = new List<Book>();

            for (var i = 0; i < booksInput.Length; i++)
            {
                var bookTokens = booksInput[i].Split();

                var edition = int.Parse(bookTokens[0]);
                var releaseDate = DateTime.ParseExact(bookTokens[1], "d/M/yyyy", CultureInfo.InvariantCulture);
                var copies = int.Parse(bookTokens[2]);
                var price = decimal.Parse(bookTokens[3]);
                var ageRestriction = int.Parse(bookTokens[4]);
                var title = string.Join(" ", bookTokens, 5, bookTokens.Length - 5);
                var category = categories[i / 10];
                var author = authors[i / 5];
